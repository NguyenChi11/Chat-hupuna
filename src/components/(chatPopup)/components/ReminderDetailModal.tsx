...
export default function ReminderDetailModal({ isOpen, message, onClose,onRefresh }: ReminderDetailModalProps) {
  ...
  useEffect(() => {
    ...
    setRepeat((message as Message & { reminderRepeat?: 'none' | 'daily' | 'weekly' | 'monthly' }).reminderRepeat || 'none');
    setEditing(false);
  }, [message]);
          ...
          {!editing ? (
            <>
              ...
              <p className="text-sm text-gray-600">
                Lặp: {(message as Message & { reminderRepeat?: 'none' | 'daily' | 'weekly' | 'monthly' }).reminderRepeat === 'daily'
                  ? 'Hàng ngày'
                  : (message as Message & { reminderRepeat?: 'none' | 'daily' | 'weekly' | 'monthly' }).reminderRepeat === 'weekly'
                  ? 'Hàng tuần'
                  : (message as Message & { reminderRepeat?: 'none' | 'daily' | 'weekly' | 'monthly' }).reminderRepeat === 'monthly'
                  ? 'Hàng tháng'
                  : 'Không lặp lại'}
              </p>
        ...
        <div className="flex gap-3 px-6 pb-6">
          {!editing ? (
            <>
              ...
              <button onClick={handleRecall} className="flex-1 cursor-pointer py-3.5 bg-red-500 hover:bg-red-600 text-white font-semibold rounded-2xl">
                 <span className="inline-flex items-center gap-2">
